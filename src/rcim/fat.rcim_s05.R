...
setwd('/Server/Meyer_Transfer_Folder/Rodrigo/D2sqs05')
...
rcim.tf.ss<-lapply(winsize, function(win.size) {
	r.cim(D2sq.s05, pheno.col=c('TF.ss'), times=400, n.marcovar=3, window=win.size, method='hk', save=TRUE)
...
save(list=c(ls(pattern='all.tf'),'rcim.tf.ss'), file="08-10-08 TF -s05 -w1,05,025.RData")
...
save(list=c(ls(pattern='all.gfp'),'rcim.gfp.ss'), file="08-10-08 GFP -s05 -w1,05,025.RData")
...
save(list=c(ls(pattern='all.rfp'),'rcim.rfp.sw'), file="08-10-08 RFP -s05 -w1,05,025.RData")
rm(list=c(ls(pattern='all.rfp'),'rcim.rfp.sw'))


rcim.ffp.ss<-lapply(winsize, function(win.size) {
	r.cim(D2sq.s05, pheno.col=c('FFP.ss'), times=400, n.marcovar=3, window=win.size, method='hk', save=TRUE)
...
save(list=c(ls(pattern='all.ffp'),'rcim.ffp.ss'), file="08-10-08 FFP -s05 -w1,05,025.RData")
rm(list=c(ls(pattern='all.ffp'),'rcim.ffp.ss'))


rcim.mfp.ss<-lapply(winsize, function(win.size) {
	r.cim(D2sq.s05, pheno.col=c('MFP.ss'), times=400, n.marcovar=3, window=win.size, method='hk', save=TRUE)
...
save(list=c(ls(pattern='all.ffp'),'rcim.ffp.ss'), file="08-10-08 MFP -s05 -w1,05,025.RData")
rm(list=c(ls(pattern='all.ffp'),'rcim.ffp.ss'))
